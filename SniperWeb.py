...
# ==========================================
st.set_page_config(page_title="Sniper v6.9 Elite", page_icon="🛡️", layout="wide")
...
class SniperEvent:
    ...
    vwap: float
    ratio: float
    ratio_yest: float  # [新增] 昨日量比
    net_10m: int
    net_1h: int
    net_day: int
    tp_price: float    # [新增] 建議停利價
    sl_price: float    # [新增] 建議停損價
...
def check_signal(pct, is_bullish, net_day, net_1h, ratio, thresholds, is_breakdown, price, vwap, has_attacked, now_time, vol_lots):
    # 庫存撤退邏輯：跌破均價 1% (User Requirement)
    if is_breakdown: return "🚨撤退"
    if pct >= 9.5: return "👑漲停"
    if now_time.time() < dt_time(9, 5): return "⏳暖機"

    # is_bullish 已經是 Price > VWAP * 1.005
    
    # [核心修改] 進攻訊號必須滿足三燈全亮:
    # 1. 價格 > 均價0.5% (is_bullish)
    # 2. 大戶1H > 0 (net_1h > 0)
    # 3. 量比 > 系統門檻 (ratio >= thresholds['tgt_ratio'])
...
"""
    ...
    for _, row in df_watch.iterrows():
        ...
        if is_bullish and not is_twii:
            tp_price = row['price'] * 1.02
            sl_price = row['vwap'] * 0.985
